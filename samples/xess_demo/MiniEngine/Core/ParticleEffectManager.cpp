...
#include "CompiledShaders/ParticleTileRender3CS.h"
#include "CompiledShaders/ParticleTileRenderFast3CS.h"
#include "CompiledShaders/ParticleTileRenderSlowDynamic3CS.h"
#include "CompiledShaders/ParticleTileRenderFastDynamic3CS.h"
#include "CompiledShaders/ParticleTileRenderSlowLowRes3CS.h"
#include "CompiledShaders/ParticleTileRenderFastLowRes3CS.h"
...
#include "CompiledShaders/ParticleEnableMaskPS.h"
...
namespace
{
    ...
    ComputePSO s_ParticleTileRenderSlowCS[3][2];	// High-Res, Low-Res, Dynamic-Res / No mask buffer, mask buffer
    ComputePSO s_ParticleTileRenderFastCS[3][2]; 	// High-Res, Low-Res, Dynamic-Res (disable depth tests) / No mask buffer, mask buffer
    ComputePSO s_ParticleDepthBoundsCS(L"Particles: Particle Depth Bounds CS");
    GraphicsPSO s_NoTileRasterizationPSO[4];
    ...
    void RenderTiles(ComputeContext& CompContext, ColorBuffer& ColorTarget, ColorBuffer& ResponsiveMaskTarget, ColorBuffer& LinearDepth, bool isUpdateResponsiveMask)
    {	
...

        {
            ...
            {
                CompContext.TransitionResource(ResponsiveMaskTarget, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);    
            }
            CompContext.TransitionResource(LinearDepth, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
            CompContext.TransitionResource(BinParticles[0], D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
            ...
            if (isUpdateResponsiveMask)
            {
                CompContext.SetDynamicDescriptor(3, 1, ResponsiveMaskTarget.GetUAV());    
            }
            ...
            CompContext.SetPipelineState(s_ParticleTileRenderSlowCS[TiledRes][isUpdateResponsiveMask ? 1 : 0]);
            CompContext.DispatchIndirect(TileDrawDispatchIndirectArgs, 0);

            CompContext.SetPipelineState(s_ParticleTileRenderFastCS[TiledRes][isUpdateResponsiveMask ? 1 : 0]);
            CompContext.DispatchIndirect(TileDrawDispatchIndirectArgs, 12);
        }
    }
    
    void RenderSprites(GraphicsContext& GrContext, ColorBuffer& ColorTarget, DepthBuffer& DepthTarget, ColorBuffer& ResponsiveMaskTarget, ColorBuffer& LinearDepth, bool isUpdateResponsiveMask)
    {
        if (EnableSpriteSort)
        {
        ...
        GrContext.SetPipelineState(s_NoTileRasterizationPSO[(EnableSpriteSort ? 0 : 1) + (isUpdateResponsiveMask ? 2 : 0)]);
        ...
        if (isUpdateResponsiveMask)
        {
            GrContext.TransitionResource(ResponsiveMaskTarget, D3D12_RESOURCE_STATE_RENDER_TARGET);
            D3D12_CPU_DESCRIPTOR_HANDLE rtvs[] { ColorTarget.GetRTV(), ResponsiveMaskTarget.GetRTV() };
            GrContext.SetRenderTargets(ARRAYSIZE(rtvs), rtvs, DepthTarget.GetDSV_DepthReadOnly());
        }
        else
        {
...
#define CreatePSO( ObjName, ShaderByteCode ) \
    ...
    {
        CreatePSO(s_ParticleTileRenderSlowCS[0][0], g_pParticleTileRender2CS);
        CreatePSO(s_ParticleTileRenderFastCS[0][0], g_pParticleTileRenderFast2CS);
        CreatePSO(s_ParticleTileRenderSlowCS[1][0], g_pParticleTileRenderSlowLowRes2CS);
        CreatePSO(s_ParticleTileRenderFastCS[1][0], g_pParticleTileRenderFastLowRes2CS);
        CreatePSO(s_ParticleTileRenderSlowCS[2][0], g_pParticleTileRenderSlowDynamic2CS);
        CreatePSO(s_ParticleTileRenderFastCS[2][0], g_pParticleTileRenderFastDynamic2CS);
        
        // Update mask pipeline, 
        // Since the sample doesn't use the RenderTiles() in particles rendering for those devices which don't support g_bTypedUAVLoadSupport_R11G11B10_FLOAT,
        // so only add the responsive mask update in the g_bTypedUAVLoadSupport_R11G11B10_FLOAT particles rendering pass.
        CreatePSO(s_ParticleTileRenderSlowCS[0][1], g_pParticleTileRender3CS);
        CreatePSO(s_ParticleTileRenderFastCS[0][1], g_pParticleTileRenderFast3CS);
        CreatePSO(s_ParticleTileRenderSlowCS[1][1], g_pParticleTileRenderSlowLowRes3CS);
        CreatePSO(s_ParticleTileRenderFastCS[1][1], g_pParticleTileRenderFastLowRes3CS);
        CreatePSO(s_ParticleTileRenderSlowCS[2][1], g_pParticleTileRenderSlowDynamic3CS);
        CreatePSO(s_ParticleTileRenderFastCS[2][1], g_pParticleTileRenderFastDynamic3CS);
    }
    else
    {
        CreatePSO(s_ParticleTileRenderSlowCS[0][0], g_pParticleTileRenderCS);
        CreatePSO(s_ParticleTileRenderFastCS[0][0], g_pParticleTileRenderFastCS);
        CreatePSO(s_ParticleTileRenderSlowCS[1][0], g_pParticleTileRenderSlowLowResCS);
        CreatePSO(s_ParticleTileRenderFastCS[1][0], g_pParticleTileRenderFastLowResCS);
        CreatePSO(s_ParticleTileRenderSlowCS[2][0], g_pParticleTileRenderSlowDynamicCS);
        CreatePSO(s_ParticleTileRenderFastCS[2][0], g_pParticleTileRenderFastDynamicCS);
    }
...
#undef CreatePSO
    ...
    DXGI_FORMAT rtv_formats[2] = { g_SceneColorBuffer.GetFormat(), g_ResponsiveMaskBuffer.GetFormat() };
    s_NoTileRasterizationPSO[2] = s_NoTileRasterizationPSO[0];
    s_NoTileRasterizationPSO[2].SetRenderTargetFormats(2, rtv_formats, g_SceneDepthBuffer.GetFormat());
    s_NoTileRasterizationPSO[2].SetPixelShader(g_pParticleEnableMaskPS, sizeof(g_pParticleEnableMaskPS));
    s_NoTileRasterizationPSO[2].Finalize();

    s_NoTileRasterizationPSO[3] = s_NoTileRasterizationPSO[2];
    s_NoTileRasterizationPSO[3].SetVertexShader(g_pParticleNoSortVS, sizeof(g_pParticleNoSortVS));
...
void ParticleEffectManager::Update(ComputeContext& Context, float timeDelta )
{
    ...
    if (timeDelta > 0.5f)    // prevent time elapse too long
    {
        timeDelta = 0.0167f;
    }
...
void ParticleEffectManager::Render(CommandContext& Context, const Camera& Camera, ColorBuffer& ColorTarget, DepthBuffer& DepthTarget, ColorBuffer& ResponsiveMaskTarget, ColorBuffer& LinearDepth, bool isUpdateResponsiveMask)
{
    if (!Enable || !s_InitComplete || ParticleEffectsActive.size() == 0)
        return;
    ...
    if (EnableTiledRendering)
    {
        ...
        RenderTiles(CompContext, ColorTarget, ResponsiveMaskTarget, LinearDepth, isUpdateResponsiveMask);
    ...
    else
    {
        ...
        RenderSprites(GrContext, ColorTarget, DepthTarget, ResponsiveMaskTarget, LinearDepth, isUpdateResponsiveMask);
    }
