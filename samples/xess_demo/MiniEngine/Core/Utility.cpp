...
std::wstring Utility::AnsiToWideString(const std::string& str)
{
    ...
    int ansiLength = static_cast<int>(str.length());
    int wideLength = MultiByteToWideChar(CP_ACP, 0, str.c_str(), ansiLength, nullptr, 0);

    std::unique_ptr<wchar_t[]> ptr(new wchar_t[wideLength]);
    MultiByteToWideChar(CP_ACP, 0, str.c_str(), ansiLength, ptr.get(), wideLength);

    return std::wstring(ptr.get(), wideLength);
}
...
std::string Utility::WideStringToAnsi(const std::wstring& wstr)
{
    ...
    int wideLength = static_cast<int>(wstr.length());
    int ansiLength = WideCharToMultiByte(CP_ACP, 0, wstr.c_str(), wideLength, nullptr, 0, nullptr, nullptr);

    std::unique_ptr<char[]> ptr(new char[ansiLength]);
    WideCharToMultiByte(CP_ACP, 0, wstr.c_str(), wideLength, ptr.get(), ansiLength, nullptr, nullptr);

    return std::string(ptr.get(), ansiLength);
}
...
std::wstring Utility::UTF8ToWideString( const std::string& str )
{
    ...
    int ansiLength = static_cast<int>(str.length());
    int wideLength = MultiByteToWideChar(CP_UTF8, 0, str.c_str(), ansiLength, nullptr, 0);

    std::unique_ptr<wchar_t[]> ptr(new wchar_t[wideLength]);
    MultiByteToWideChar(CP_UTF8, 0, str.c_str(), ansiLength, ptr.get(), wideLength);
...
std::string Utility::WideStringToUTF8( const std::wstring& wstr )
{
    ...
    int wideLength = static_cast<int>(wstr.length());
    int ansiLength = WideCharToMultiByte(CP_UTF8, 0, wstr.c_str(), wideLength, nullptr, 0, nullptr, nullptr);

    std::unique_ptr<char[]> ptr(new char[ansiLength]);
    WideCharToMultiByte(CP_UTF8, 0, wstr.c_str(), wideLength, ptr.get(), ansiLength, nullptr, nullptr);
...
std::wstring Utility::GetProgramDirectory()
{
    wchar_t modulePath[MAX_PATH];
    modulePath[0] = 0;
    GetModuleFileNameW(nullptr, modulePath, MAX_PATH);
    return GetBasePath(modulePath);
}

std::wstring Utility::GetWorkingDirectory()
{
...
void Utility::SetWorkingDirectory(const std::wstring& dir)
{
    SetCurrentDirectory(dir.c_str());
}

uint32_t Utility::GetThreadId()
{
