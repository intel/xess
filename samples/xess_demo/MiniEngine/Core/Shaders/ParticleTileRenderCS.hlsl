...
RWTexture2D<float> g_ResponsiveMaskBuffer : register(u1);
#endif
#else
...
#ifdef ENABLE_PARTICLE_RESPONSIVE_MASK
    float3 responsiveColor = ceil(Color.rgb);
    g_ResponsiveMaskBuffer[ST] = max(responsiveColor.r, max(responsiveColor.g, responsiveColor.b));
#endif
#else
