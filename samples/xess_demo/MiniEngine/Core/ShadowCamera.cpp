    ...
    uint32_t BufferWidth, uint32_t BufferHeight, uint32_t BufferPrecision)
{
    ...
    XMMATRIX projMat = XMMatrixOrthographicOffCenterRH(-ShadowBounds.GetX(), ShadowBounds.GetX(), 
        -ShadowBounds.GetY(), ShadowBounds.GetY(), 
        ShadowBounds.GetZ(), -ShadowBounds.GetZ()); // invert Z

    SetProjMatrix( Matrix4(projMat) );
