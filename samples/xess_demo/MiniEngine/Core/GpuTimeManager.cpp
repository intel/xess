...
/*
    Resolving incomplete or uninitialized queries is undefined behavior because the driver might internally store GPUVAs or other data within unresolved queries.
    And so attempting to resolve these queries on uninitialized data could cause a page fault or device hang. 
    Older versions of the debug layer didn't validate this behavior.
    https://docs.microsoft.com/zh-tw/windows/win32/api/d3d12/nf-d3d12-id3d12graphicscommandlist-resolvequerydata

    xing: So manual patch here to prevent this behavior which causes a debuger layer error below in WIN11
    "D3D12 ERROR: ID3D12CommandQueue::ExecuteCommandLists: Cannot Resolve query that has never been performed for Index [*] in QueryHeap (0x00000207E7A19160:'GpuTimeStamp QueryHeap') in command list (0x00000207E76812A0:'CommandList'). [ EXECUTION ERROR #1319: RESOLVE_QUERY_INVALID_QUERY_STATE]"
 */
#define PATCH_UNINITIALIZED_QUERIES
...
#ifdef PATCH_UNINITIALIZED_QUERIES
    uint32_t m_LastTimerIdx = UINT32_MAX;
#endif
}
...
{
#ifdef PATCH_UNINITIALIZED_QUERIES
    if (m_LastTimerIdx + 1 < TimerIdx)
    {
        for (uint32_t i = m_LastTimerIdx + 1; i < TimerIdx; i++)
        {
            Context.InsertTimeStamp(sm_QueryHeap, i * 2);
            Context.InsertTimeStamp(sm_QueryHeap, i * 2 + 1);
        }
    }
    m_LastTimerIdx = TimerIdx;
#endif
