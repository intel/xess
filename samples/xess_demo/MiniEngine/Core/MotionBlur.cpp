...
#include "CompiledShaders/MotionBlurPrePassUnpackedVelocityCS.h"
#include "CompiledShaders/MotionBlurFinalPassUnpackedVelocityCS.h"
#include "CompiledShaders/MotionBlurFinalPassUnpackedVelocityPS.h"
...
namespace MotionBlur
{
    ...
    ComputePSO s_MotionBlurPrePassCS_UnpackedVelocity(L"Motion Blur: Motion Blur Pre-Pass CS - Unpacked Velocity");
    ComputePSO s_MotionBlurFinalPassCS_UnpackedVelocity(L"Motion Blur: Motion Blur Final Pass CS - Unpacked Velocity");
    GraphicsPSO s_MotionBlurFinalPassPS_UnpackedVelocity(L"Motion Blur: Motion Blur Final Pass PS - Unpacked Velocity");
...
#define CreatePSO( ObjName, ShaderByteCode ) \
    ...
    if (g_bTypedUAVLoadSupport_R11G11B10_FLOAT)
    {
        ...
        CreatePSO(s_MotionBlurFinalPassCS_UnpackedVelocity, g_pMotionBlurFinalPassUnpackedVelocityCS);
    }
    else
    {
        ...
        s_MotionBlurFinalPassPS_UnpackedVelocity.SetRootSignature(g_CommonRS);
        s_MotionBlurFinalPassPS_UnpackedVelocity.SetRasterizerState(RasterizerTwoSided);
        s_MotionBlurFinalPassPS_UnpackedVelocity.SetBlendState(BlendPreMultiplied);
        s_MotionBlurFinalPassPS_UnpackedVelocity.SetDepthStencilState(DepthStateDisabled);
        s_MotionBlurFinalPassPS_UnpackedVelocity.SetSampleMask(0xFFFFFFFF);
        s_MotionBlurFinalPassPS_UnpackedVelocity.SetInputLayout(0, nullptr);
        s_MotionBlurFinalPassPS_UnpackedVelocity.SetPrimitiveTopologyType(D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE);
        s_MotionBlurFinalPassPS_UnpackedVelocity.SetVertexShader(g_pScreenQuadCommonVS, sizeof(g_pScreenQuadCommonVS));
        s_MotionBlurFinalPassPS_UnpackedVelocity.SetPixelShader(g_pMotionBlurFinalPassUnpackedVelocityPS, sizeof(g_pMotionBlurFinalPassUnpackedVelocityPS));
        s_MotionBlurFinalPassPS_UnpackedVelocity.SetRenderTargetFormat(g_SceneColorBuffer.GetFormat(), DXGI_FORMAT_UNKNOWN);
        s_MotionBlurFinalPassPS_UnpackedVelocity.Finalize();
    }
    ...
    CreatePSO(s_MotionBlurPrePassCS_UnpackedVelocity, g_pMotionBlurPrePassUnpackedVelocityCS);
...
void MotionBlur::RenderObjectBlur( CommandContext& BaseContext, ColorBuffer& velocityBuffer )
{
    ...
    if (isUpscalingMode)
    {
        ...
        Context.TransitionResource(g_MotionPrepBuffer, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        Context.TransitionResource(g_UpscaledSceneColorBuffer, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        Context.TransitionResource(g_UpscaledVelocityBuffer, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);

        Context.SetDynamicDescriptor(2, 0, g_MotionPrepBuffer.GetUAV());
        Context.SetDynamicDescriptor(1, 0, g_UpscaledSceneColorBuffer.GetSRV());
        Context.SetDynamicDescriptor(1, 1, g_UpscaledVelocityBuffer.GetSRV());

        Context.SetPipelineState(s_MotionBlurPrePassCS_UnpackedVelocity);
        Context.Dispatch2D(g_MotionPrepBuffer.GetWidth(), g_MotionPrepBuffer.GetHeight());

        if (g_bTypedUAVLoadSupport_R11G11B10_FLOAT)
        {
            Context.SetPipelineState(s_MotionBlurFinalPassCS_UnpackedVelocity);

            Context.TransitionResource(g_UpscaledSceneColorBuffer, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
            Context.TransitionResource(g_UpscaledVelocityBuffer, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
            Context.TransitionResource(g_MotionPrepBuffer, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);

            Context.SetDynamicDescriptor(2, 0, g_UpscaledSceneColorBuffer.GetUAV());
            Context.SetDynamicDescriptor(1, 0, g_UpscaledVelocityBuffer.GetSRV());
            Context.SetDynamicDescriptor(1, 1, g_MotionPrepBuffer.GetSRV());
            Context.SetConstants(0, 1.0f / Width, 1.0f / Height);

            Context.Dispatch2D(Width, Height);

            Context.InsertUAVBarrier(g_UpscaledSceneColorBuffer);
        }
        else
        {
            GraphicsContext& GrContext = BaseContext.GetGraphicsContext();
            ...
            GrContext.SetRootSignature(g_CommonRS);
            GrContext.SetPipelineState(s_MotionBlurFinalPassPS_UnpackedVelocity);

            GrContext.TransitionResource(g_UpscaledSceneColorBuffer, D3D12_RESOURCE_STATE_RENDER_TARGET);
            GrContext.TransitionResource(g_UpscaledVelocityBuffer, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
            GrContext.TransitionResource(g_MotionPrepBuffer, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);

            GrContext.SetDynamicDescriptor(1, 0, g_UpscaledVelocityBuffer.GetSRV());
            GrContext.SetDynamicDescriptor(1, 1, g_MotionPrepBuffer.GetSRV());
            GrContext.SetConstants(0, 1.0f / Width, 1.0f / Height);
            GrContext.SetRenderTarget(g_UpscaledSceneColorBuffer.GetRTV());
            GrContext.SetViewportAndScissor(0, 0, Width, Height);
