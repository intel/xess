...
// Supported DPI awareness, fullscreen toggle and ImGui message forward.
...
namespace GameCore
{
    ...
    bool UpdateApplication( IGameApp& game )
    {
        ...
        UiContext.SetRenderTarget(g_OverlayBuffer.GetRTV());
        UiContext.SetViewportAndScissor(0, 0, g_OverlayBuffer.GetWidth(), g_OverlayBuffer.GetHeight());
        EngineTuning::Display( UiContext, 10.0f, 40.0f, (float)g_DisplayWidth, (float)g_DisplayHeight);
    ...
    {
        // ImGuiModule handles windows messages first.
        if (ImGuiModule::WinProcHandler(hWnd, message, wParam, lParam))
            return 0;
        ...
        case WM_DPICHANGED:
        {
            Display::UpdateDPIScale();
            break;
        }
        // Subclass of IGameApp will handle the size change.
        //case WM_SIZE:
