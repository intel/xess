...
void PostEffects::GenerateBloom( ComputeContext& Context )
{
...

    {
        ...
        if (isUpscalingMode)
        {
            Context.SetDynamicDescriptor(2, 0, g_UpscaledSceneColorBuffer.GetSRV());
        }
        else
        {
...
void PostEffects::ProcessHDR( ComputeContext& Context )
{
    ...
    if (Graphics::IsUpscalingEnabled())
    {
        if (g_bTypedUAVLoadSupport_R11G11B10_FLOAT)
        ...
        // Set constants
        Context.SetConstants(0, 1.0f / g_UpscaledSceneColorBuffer.GetWidth(), 1.0f / g_UpscaledSceneColorBuffer.GetHeight(),
            (float)BloomStrength);
        ...
        if (g_bTypedUAVLoadSupport_R11G11B10_FLOAT)
            Context.SetDynamicDescriptor(1, 0, g_UpscaledSceneColorBuffer.GetUAV());
        else
        {
            Context.SetDynamicDescriptor(1, 0, g_PostEffectsBuffer.GetUAV());
            Context.SetDynamicDescriptor(2, 2, g_UpscaledSceneColorBuffer.GetSRV());
        }
        ...
        Context.Dispatch2D(g_UpscaledSceneColorBuffer.GetWidth(), g_UpscaledSceneColorBuffer.GetHeight());
    }
    else
    {
        if (g_bTypedUAVLoadSupport_R11G11B10_FLOAT)
...
void PostEffects::Render( void )
{
    ...
    if (Graphics::IsUpscalingEnabled())
    {
        Context.TransitionResource(g_UpscaledSceneColorBuffer, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
    }
    else
    {
