...
{
    if (!ImGuiModule::WantCaptureKeyboard()
        && (GameInput::IsFirstPressed( GameInput::kBackButton )
...
void EngineTuning::Display( GraphicsContext& Context, float x, float y, float w, float h )
{
    ...
    */
    Context.SetScissor((uint32_t)Floor(x), (uint32_t)Floor(y),
        (uint32_t)Ceiling(x + w), (uint32_t)Ceiling(y + h));
