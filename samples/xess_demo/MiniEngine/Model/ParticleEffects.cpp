...
void ParticleEffects::InitFromJSON(const wstring& InitJsonFile, const wstring& AssetsRootDir)
{
    ...
    for (auto& emitter : particle_setup["ParticleEmitters"])
    {
        ...
        for (json::iterator it = emitter.begin(); it != emitter.end(); ++it)
        {
            ...
            {
                Effect.TexturePath = AssetsRootDir + L"/" + MAKE_WSTRING(it);
            }
            ...
            else if (it.key() == "EmitProperties")
            {
                for (json::iterator it2 = it.value().begin(); it2 != it.value().end(); ++it2)
                {
                    ...
                    {
                        LOG_WARNF("Warning:  Emitter property \"%s\" not found.", it2.key().c_str());
                    }
                }
            }
            else
            {
                LOG_WARNF("Warning:  Effect property \"%s\" not found.", it.key().c_str());
            }
...
void ParticleEffects::ClearTexturePool()
{
    ClearEffectTexturePool();
}
