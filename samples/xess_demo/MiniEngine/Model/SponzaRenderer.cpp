...
namespace Sponza
{
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE m_SamplerHandler;
    ...
    void CreateDiffuseSampler(float LodBias);
}
...
void Sponza::Startup( Camera& Camera )
{
    ...
    ParticleEffects::InitFromJSON(L"Sponza/particles.json", L"");
...
void Sponza::CreateDiffuseSampler(float MipBias)
{
    SamplerDesc diffuseSamplerDesc;
    diffuseSamplerDesc.MaxAnisotropy = 8;
    diffuseSamplerDesc.MipLODBias = MipBias;

    m_SamplerHandler = diffuseSamplerDesc.CreateDescriptor();
    ...
    float MipBias)
{
    ...
    if (MipBias != m_MipBias)
    {
        CreateDiffuseSampler(MipBias);
    }

    // Here we use the first sampler of kMaterialSamplers
    gfxContext.SetDynamicSampler(3, 0, m_SamplerHandler);
    ...
    if (!skipDiffusePass)
    {
        Lighting::FillLightGrid(gfxContext, camera, false);
