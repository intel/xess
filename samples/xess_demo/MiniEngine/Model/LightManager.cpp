...
// Forward+ lighting support improved.
...
#include "CompiledShaders/FillLightGrid8CS.h"
#include "CompiledShaders/FillLightGrid16CS.h"
#include "CompiledShaders/FillLightGrid24CS.h"
#include "CompiledShaders/FillLightGrid32CS.h"
...
__declspec(align(16)) struct LightData
{
    ...
    float padding[3]; // Padding so the structure is 16-byte aligned.
};
...
namespace Lighting
{
    ...
    ByteAddressBuffer m_LightGridTransparent;
    ByteAddressBuffer m_LightGridBitMaskTransparent;
...
void Lighting::InitializeResources( void )
{
    ...
    m_FillLightGridCS_16.SetComputeShader(g_pFillLightGrid16CS, sizeof(g_pFillLightGrid16CS));
    m_FillLightGridCS_16.Finalize();

    m_FillLightGridCS_24.SetRootSignature(m_FillLightRootSig);
    m_FillLightGridCS_24.SetComputeShader(g_pFillLightGrid24CS, sizeof(g_pFillLightGrid24CS));
    m_FillLightGridCS_24.Finalize();

    m_FillLightGridCS_32.SetRootSignature(m_FillLightRootSig);
    m_FillLightGridCS_32.SetComputeShader(g_pFillLightGrid32CS, sizeof(g_pFillLightGrid32CS));
    ...
    m_LightGridTransparent.Create(L"m_LightGrid Alpha", lightGridSizeBytes, 1);
    ...
    m_LightGridBitMaskTransparent.Create(L"m_LightGridBitMask Alpha", lightGridBitMaskSizeBytes, 1);
...
void Lighting::CreateRandomLights( const Vector3 minBound, const Vector3 maxBound )
{
    ...
    for (uint32_t n = 0; n < MaxLights; n++)
    {
        ...
        if (n < 32 * 1)
            type = 0;
        //else if (n < 32 * 3) // We make all spotlights with shadow
        //    type = 1;
        else
            type = 2;
...
void Lighting::Shutdown(void)
{
    ...
    m_LightGridTransparent.Destroy();
    m_LightGridBitMaskTransparent.Destroy();
    m_LightShadowArray.Destroy();
    m_LightShadowTempBuffer.Destroy();
}

void Lighting::FillLightGrid(GraphicsContext& gfxContext, const Camera& camera, bool transparent)
{
    ...
    if (transparent)
    {
        Context.TransitionResource(m_LightGridTransparent, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        Context.TransitionResource(m_LightGridBitMaskTransparent, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);

        Context.SetDynamicDescriptor(2, 0, m_LightGridTransparent.GetUAV());
        Context.SetDynamicDescriptor(2, 1, m_LightGridBitMaskTransparent.GetUAV());
    }
    else
    {
    ...
    csConstants.Alpha = transparent ? 1 : 0;
    ...
    if (transparent)
    {
        Context.TransitionResource(m_LightGridTransparent, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
        Context.TransitionResource(m_LightGridBitMaskTransparent, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
    }
    else
    {
